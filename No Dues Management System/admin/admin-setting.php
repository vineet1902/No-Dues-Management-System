<?php
	...
	else if(strcmp($_SESSION['login_as'],"manager")==0){
		header('location:index.php');
	}
	if(isset($_POST['password'])){
		...
		if(strcmp($_SESSION['password'],$pass)==0){
			if(strcmp($pass1, $pass2)==0){
				...
				//$query = mysql_query("update admin set password='".$pass1."' where username='".$_SESSION['username']."'",$connection);
				$query = mysql_query("update admin set password='$pass1' where username='admin'",$connection);
				/*if(mysql_num_rows($query)!=0){
					echo 'Hello';
				}*/
				//$query = mysql_query("update admin set password = '$pass1' where username='".$_SESSION['username']."'",$connection);
				//$query1 = mysql_query("update admin set password='$pass1' where username='".$_SESSION['username']."'",$connection);
				if($query){
					$_SESSION['password'] = $pass1;
					header('location:admin-setting.php?error=none');
				}
				else{
					header('location:admin-setting.php?error=connection');
				}
			}
			else{
				header('location:admin-setting.php?error=pass_match');
			}
		}
		else{
			header('location:admin-setting.php?error=cur_pass');
		}
	}
?>
