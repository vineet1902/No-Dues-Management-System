<?php
	...
	else if(strcmp($_SESSION['login_as'],"admin")==0){
		header('location:index.php');
	}
		...
		if(strcmp($_SESSION['password'],$pass)==0){
			if(strcmp($pass1, $pass2)==0){
				...
				if($query){
					...
					header('location:manager-setting.php?error=connection');
				}
			}
			else{
				header('location:manager-setting.php?error=pass_match');
			}
		}
		else{
			header('location:manager-setting.php?error=cur_pass');
		}
	}
?>
...
<html lang="en">
  ...
  <body style="background-color:;">
	  ...
	  <div class="container">
	  	<h3 class="navbar-text"><a href="manager-profile.php"><span class="glyphicon glyphicon-home" title="Home" aria-hidden="true"></span></a></h3>
	...
	<?php
		if(isset($_GET['error'])){
			...
			if(strcmp($_GET['error'],"connection")==0){
				$msg = "Connection problem. Try again. If problem persists, try a different password.";
			}
